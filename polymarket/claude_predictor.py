...
class ClaudePredictor:
    ...
    def _call(self, summaries: list[tuple[str, str]]) -> dict[str, str]:
        ...
        prompt = (
            "You are a short-horizon quant for a Polymarket-mirror prediction "
            "market on Vision L3. Each asset is a Polymarket event whose "
            "midprice is fed on-chain. For each asset below, output a single "
            "token UP or DOWN predicting whether its value will be higher at "
            "the next 60-second tick. Reply with ONLY a JSON object mapping "
